...
La operación termina correctamente cuando ambos stacks no existen y el bucket `roadmap2u-<stage>-765932874577` tampoco existe. El log group `/aws/apigateway/roadmap-api-<stage>` permanece intencionalmente porque pertenece al toolkit protegido, no al workload. Conserva eventos `DELETE_FAILED`, errores de objetos y el último estado; corrige la causa concreta y reanuda, sin desactivar termination protection de los toolkits ni ampliar el rol.
