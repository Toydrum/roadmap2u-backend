...
El permissions boundary del runtime se transiciona por stage y distingue si realmente existe un secreto anterior. Durante el corte actual, `dev` permite como máximo leer el parámetro SSM exacto y su secreto retenido limitado por cuenta, región, stage y el prefijo físico `v1-*`; `test`, inicializado sin secreto legado, permite únicamente el parámetro SSM exacto; `prod` continúa permitiendo sólo Secrets Manager hasta que llegue su ventana. El boundary no concede permisos por sí mismo: los runtimes finales de `dev` y `test` conservan únicamente la identity policy de SSM. La autoridad CloudFormation para administrar secretos se retira de `dev` y `test`, y se conserva en `prod` mientras ese stage siga usando su template legado. El manifiesto describe el provider real del artefacto —este template es SSM en las tres etapas— y el workflow lo compara con el control plane actual: permite `dev` y `test`, y bloquea su promoción a `prod`. Un rollback de un SHA legacy sin manifiesto requiere una atestación owner-issued, inmutable para el rol de deploy, que identifique su provider exacto; nunca se infiere sólo por tener un release marker. Antes de migrar otro stage, actualiza primero el control plane y después el gate esperado de ese stage. Retira definitivamente los permisos legados cuando cada stage haya terminado su ventana de observación.
...
Si falla la validación después del cambio, apaga emisión y canje. En `dev`, conserva ambos almacenes y corrige IAM o restaura el parámetro desde el secreto retenido mediante un cambio revisado; no pegues el JSON en la terminal. En `test` no existe un secreto legado: no crees uno como atajo, aplica un forward-fix compatible con SSM o una restauración revisada de una versión conocida del parámetro. El secreto retenido de `dev` ya no pertenece a la plantilla final, por lo que no se debe asumir que redesplegar directamente la plantilla antigua lo volverá a adoptar. Para un stage migrado, el workflow exige un artefacto `ssm-secure-string-v1` y rechaza como rollback cualquier SHA legacy. Los stages cuyo control plane sigue en Secrets Manager sólo admiten SHAs legacy con marcador y atestación exacta, y rechazan este artefacto SSM mientras no estén preparados.
